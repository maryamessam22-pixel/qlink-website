...
import {
  ...
  Truck, ShieldCheck, Undo2, CheckCircle2
} from 'lucide-react';
...
const InTheBoxPulse = () => {
  ...
  const boxItems = [
    { name: t('pulseDetails.inboxItem1Name'), sub: t('pulseDetails.inboxItem1Sub') },
    { name: t('pulseDetails.inboxItem2Name'), sub: t('pulseDetails.inboxItem2Sub') },
    { name: t('pulseDetails.inboxItem3Name'), sub: t('pulseDetails.inboxItem3Sub') },
    { name: t('pulseDetails.inboxItem4Name'), sub: t('pulseDetails.inboxItem4Sub') },
  ];
  ...
  return (
    <div className={`subpage-container ${isAr ? 'rtl-text' : ''}`} dir={isAr ? 'rtl' : 'ltr'}>
      <SEO
        title={isAr ? 'بالس - محتويات العلبة' : 'Pulse – In The Box'}
        description={isAr ? 'محتويات علبة كيو لينك بالس' : "What's in the box for Qlink Pulse."}
        slug="shop/pulse/inbox"
      />
      <DynamicBackground />
      ...
      <div className="subpage-wrapper">
        ...
        <div className="subpage-tabs scroll-animate stag-3">
          <div className="tab-headers">
            ...
            <button className="tab-btn" onClick={() => goTab('/shop/pulse/privacy')}>{t('pulseDetails.tabPrivacy')}</button>
            <button className="tab-btn active">{t('pulseDetails.tabInbox')}</button>
          </div>
